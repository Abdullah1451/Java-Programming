...
public class Stegano
{
	...
	private byte[] decode_text(byte[] image)
	{
		...
		for(int i=0; i<32; ++i) //i=24 will also work, as only the 4th byte contains real data
		{
			...
			System.out.println("length  : "+image[i]);
		}
	...
	public static void main(String[] args){
		...
		if(option == 1){
			...
			String newImageName = input.next();

			System.out.print("\nEnter Secret Message  :: ");
			String secretMessage = input.next();

			shi.encode(path, imageName, imageType, newImageName, secretMessage);
		}
		else if(option == 2){
			System.out.print("\n\nEnter The Path Of Image Folder  :: ");
			String path = "D:\\Abdullah Working\\Java\\Steganography In Java";
